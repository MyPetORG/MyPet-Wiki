# Main Configuration

The _config.yml_ file is the main configfile of _MyPet_. All pet type related options can be found in the [pet-config.yml](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/petconfig/README.md).

| Setting | Type | Default | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | Description |  |
| :--- | :---: | :---: | :---: | ---: | :--- |
| **MyPet:** |  |  |  |  |  |
| **Miscellaneous Settings** |  |  |  |  |  |
|   **Leash:** |  |  |  |  |  |
|     Consume: | boolean | `false` | - | Enable to consume the leash item when a new pet is leashed. |  |
|   OwnerCanAttackPet: | boolean | `false` | - | Enable to allow the owner to hit his own pet. |  |
|   DisablePetVersusPlayer: | boolean | `false` | - | Prevents combat between players and pets. |  |
|   RemovePetsAfterRelease: | boolean | `false` | - | Enable to remove pets when they get released. Prevents players from griefing other players by releasing monsters in their vicinity. |  |
|   ReleasePetsOnDeath: | boolean | `false` | - | Enable to release the pet when it dies. Player that have the _MyPet.admin_ permission are excluded from this. |  |
|   FollowStartDistance: | double | `7.0` | - | Sets the distance from the player where the pet starts to follow him. |  |
|   RetainEquipmentOnTame: | boolean | `true` | - | Allows mobs to keep their equipment after leashed \(based on the drop chance\). |  |
|   Make-Pet-Invisible-When-Owner-Is-Invisible: | boolean | `true` | - | Makes the pet invisible when the owner has the `Invisible` effect. Does not affect vanished players. |  |
|   **Log:** |  |  |  |  |  |
|     Level: | string | `INFO` | - | Set the level at which the messages will be logged to the file. All possible log levels can be found [here](https://docs.oracle.com/javase/7/docs/api/java/util/logging/Level.html#field_summary). |  |
|   **Info:** |  |  |  |  |  |
|     Wiki-URL: | string | `https://wiki.mypet-plugin.de` | - | This can be changed if the server has it's own Wiki for MyPet. |  |
|   Max-Stored-Pet-Count: | integer | `45` | - | Sets the maximum amount of inactive \(stored\) pets a player can have. |  |
|   **Update:** |  |  |  |  |  |
|    Check: | boolean | `true` | - | Sets if the plugin will check for updates when it is loaded. This will not download the new version. |  |
|    Download: | boolean | `false` | - | Sets if the plugin will download the update. |  |
|    ReplaceOld: | boolean | `false` | - | Sets if the plugin will load the update on the next server start. |  |
|    Token: | String | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The download token you need to download the latest MyPet-Premium version. You can get your token [here](https://mypet-plugin.de/download). |  |
|   Activate-Resourcepack-By-Default: | boolean | `false` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | This will actiave the ressource pack by default. Players can still disable it. |  |
|   Throw-PlayerMoveEvent-While-Riding: | boolean | `true` | - | Disable this when other plugins cause bugs  because of the thrown events. |  |
|   OverwriteLanguages: | string | `''` | - | If you don't want per player language detection you can use this to overwrite the language for all players. Available languages can be found [here](https://github.com/xXKeyleXx/MyPet-Translations). Example: `pt_br` |  |
| **Repository Settings** |  |  |  |  |  |
|   **Repository:** |  |  |  |  |  |
|     Type: | string | `SQLite` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The storage type where the plugin will save the pets into. ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) Premium users also have access to `MySQL` and `MongoDB`. |  |
|     ConvertFrom: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | This options allows to migrate from one storage type to another. For example from `NBT` to `MySQL`. |  |
|     **MySQL:** |  |  |  |  |  |
|       Database: | string | `mypet` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The name of the MySQL database. |  |
|       TablePrefix: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The table prefix if the database is shared with other applications. |  |
|       Host: | string | `localhost` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The address of the MySQL server. |  |
|       Port: | integer | `3306` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The port of the MySQL server. |  |
|       Password: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The password of the MySQL user. |  |
|       User: | string | `root` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The username of the MySQL user. |  |
|       MaxConnections: | integer | `CPU cores * 2` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The amount of simultaneous connections the plugin will create to the MySQL server. You can find more about the best pool size [here](https://github.com/brettwooldridge/HikariCP/wiki/About-Pool-Sizing). |  |
|     **MongoDB:** |  |  |  |  |  |
|       Database: | string | `mypet` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The name of the MongoDB database. |  |
|       CollectionPrefix: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The collection prefix if the database is shared with other applications. |  |
|       Host: | string | `localhost` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The address of the MongoDB server. |  |
|       Port: | integer | `3306` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The port of the MongoDB server. |  |
|       Password: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The password of the MongoDB user. |  |
|       User: | string | `''` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | The username of the MongoDB user. |  |
| **Respawn Settings** |  |  |  |  |  |
|   **Respawn:** |  |  |  |  |  |
|     **Time:** |  |  |  |  |  |
|       **Default:** |  |  |  |  |  |
|         Factor: | integer | `5` | - | Determines how long the owner has to wait until the pet respawns when it **wasn't** killed by a player.  `Respawntime = Factor * (Level of MyPet) + Fixed` |  |
|         Fixed: | integer | `0` | - | See `Factor` |  |
|       **Player:** |  |  |  |  |  |
|         Factor: | integer | `5` | - | Determines how long the owner has to wait until the pet respawns when it **was** killed by a player.  `Respawntime = Factor * (Level of MyPet) + Fixed` |  |
|         Fixed: | integer | `0` | - | See `Factor` |  |
|       **EconomyCost:** |  |  |  |  |  |
|         Factor: | double | `5` | - | Determines how much it will cost if you want to revive a dead MyPet.  `Costs = Factor * (Respawn Time in sec.) + Fixed` |  |
|         Fixed: | double | `0` | - | See `Factor` |  |
| **Permission Settings** |  |  |  |  |  |
|   **Permissions:** |  |  |  |  |  |
|     Enabled: | boolean | `true` | - | Disable the use of [permissions](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/permissions/README.md) and fall back to the OP permission system. |  |
|     Extended: | boolean | `false` | - | Enable if you want to use some addition permissions that can be found [here](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/permissions/README.md#extended_mypet_permissions). |  |
|     Legacy: | boolean | `false` | - | Enable if you want to use the pre 2.0.0 permissions. |  |
| **Level System Settings** |  |  |  |  |  |
|   **LevelSystem:** |  |  |  |  |  |
|     CalculationMode: | string | `Default` | - | Set this to `JS` or `JavaScript` if you want use a custom [exp.js](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/expjs/README.md). |  |
| **Hunger System Settings** |  |  |  |  |  |
|   **HungerSystem:** |  |  |  |  |  |
|     Active: | boolean | `true` | - | Disable the [hungersystem](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/hungersystem/README.md) if you don't want your pets need food to survive. |  |
|     Time: | integer | `60` | - | Sets the interval \(in seconds\) in which the hunger counter will be reduced by `1`. |  |
|     HungerPointsPerFeed: | double | `6.0` | - | Sets the value the hunger counter will be increased by if the pet is fed. |  |
| **Skilltree Settings** |  |  |  |  |  |
|   **Skilltree:** |  |  |  |  |  |
|     AutomaticAssignment: | boolean | `false` | - | Enable to automatically assign a skilltree to a pet when it is leashed. |  |
|     RandomAssignment: | boolean | `false` | - | Like `AutomaticAssignment` but the skilltree is selected randomly. |  |
|     InheritAlreadyInheritedSkills: | boolean | `false` | - | Enable this if you want skill that are inherited by another skilltree can be inherited again. |  |
|     ChooseOnce: | boolean | `false` | - | Enable this if players shouldn't be able to pick another skilltree once the pet has a skilltree. |  |
|     PreventLevellingWithout: | boolean | `true` | - | Pets without a skilltree will not gain XP if it is set to `true`. |  |
|     **SwitchFee:** |  |  |  |  |  |
|       Admin: | boolean | `false` | - | Set this to `true` if admins should pay the same skilltree switch penalty like normal players. |  |
|       Percent: | integer | `5` | - | The percentage of XP a players has to pay if he switches to another skilltree. |  |
|       Fixed: | double | `0.0` | - | The amount of XP a players has to pay if he switches to another skilltree. |  |
| **Name Settings** |  |  |  |  |  |
|   **Name:** |  |  |  |  |  |
|     Filter: | list | `whore, fuck` | - | Every pet name is checked against this list of filters \(string/regular expression\). |  |
|     MaxLength: | integer | `32` | - | The maximum length a petname can have. |  |
|     **Tag:** |  |  |  |  |  |
|       Show: | boolean | `true` | - | Set this to `false` if you don't want nametags for MyPets. |  |
|       Prefix: | string | `<aqua>` | - | This text will be added in front of the name. You can use color codes and these placeholders:  `<owner>` ⇒ name of the owner  `<level>` ⇒ level of the pet |  |
|       Suffix: | string | `''` | - | This text will be added the end of the name. You can use color codes and these placeholders:  `<owner>` ⇒ name of the owner  `<level>` ⇒ level of the pet |  |
| **Experience Settings** |  |  |  |  |  |
|   **Exp:** |  |  |  |  |  |
|     DamageWeightedExperienceDistribution: | boolean | `false` | - | This setting allows pets to gain XP even if they did not kill the enemy. Every bit of damage is counted and when the enemy dies the XP will be split up to reflect the damage given to that enemy. So if a pet does 50% of the damage it will gain 50% of the total XP. |  |
|     **Passive:** |  |  |  |  |  |
|       Always-Grant-Passive-XP: | boolean | `false` | - | This setting allows the pet to always gain XP when the owner kills a monster, even if it has a damage skill \([Damage](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/damage/README.md) or [Ranged](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/ranged/README.md)\). |  |
|       PercentPerMonster: | integer | `25` | - | Sets the percentage of XP a pet will get when the owner kills an enemy. |  |
|     **Loss:** |  |  |  |  |  |
|       Drop: | boolean | `true` | - | If this setting is set to `true` the lost XP is dropped on the ground and can be picked up by others. |  |
|       Percent: | integer | `0` | - | Sets the percentage of XP a pet will lose when it dies. |  |
|       Fixed: | double | `0.0` | - | Sets the XP a pet will lose when it dies. |  |
|     **Gain:** |  |  |  |  |  |
|       PreventFromSpawnReason: | list | `[]` | - | This setting is a list of spawn reasons. Every mob that is spawned by any of these spawn reasons will not grant any XP. All spawn reasons can be found [here](https://hub.spigotmc.org/javadocs/bukkit/org/bukkit/event/entity/CreatureSpawnEvent.SpawnReason.html#enum_constant_summary). **This resets on every server reload/restart so it will not prevent it after them!** |  |
|     LevelCap: | integer | `100` | - | A pet can not gain any XP and level up if that level is reached. |  |
|     **Active:** |  |  |  |  |  |
|     [**ENTITY\_TYPE**](https://hub.spigotmc.org/javadocs/bukkit/)**:** |  |  |  |  |  |
|       Min: | double | `5.0` | - | The minimal amount of XP that this entity type grants. |  |
|       Max: | double | `5.0` | - | The maximum amount of XP that this entity type grants. |  |
|  |  | **Skill Settings** |  |  |  |
|   **Skill:** |  |  |  |  |  |
|     [**Control**](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/control/README.md)**:** |  |  |  |  |  |
|       Item: | string | `420` | - | Sets the item that allows the player to use the [Control](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/control/README.md) skill the pet. This setting follows the [config item](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/configitem/README.md) guidelines |  |
|     **\[Rride\]\(skills/ride\]\]:** |  |  |  |  |  |
|       Item: | string | `420` | - | Sets the item that allows the player to mount the pet. This setting follows the [config item](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/configitem/README.md) guidelines |  |
|       HungerPerMeter: | double | `0.01` | - | If the [Hunger-System](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/hungersystem/README.md) is active, this setting set the value the hunger value is decreased by for every ridden meter. |  |
|       **FlyZones:** |  |  |  |  |  |
|         **SomeWorld**::**SomeRegion**:          **SomeWorld**::**global**:          **…** | boolean | `true` | ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) | This setting allows to disable flying in WorldGuard regions. For every region you want to allow/disable flying you have to add a new line that looks like this:  ::: false  Regions with higher priorities will overwrite regions with a lower priorities. This allows to have small regions that allow flying in big regions where flying is diabled and vice versa.  An example can be found [here](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/ride/README.md#example). |  |
|     \*\*\[\[skills:backpack | Inventory\]\]:\*\* |  |  |  |  |
|       Creative: | boolean | `false` | - | Allows players to open the inventory of their pet when they are in creative mode. |  |
|       DropWhenOwnerDies: | boolean | `false` | - | When this is set to `true` the pet will drop the content in it's inventory when the owner dies. |  |
|     [**Beacon**](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/skills/beacon/README.md)**:** |  |  |  |  |  |
|       HungerDecreaseTime: | integer | `100` | - | If the [Hunger-System](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/hungersystem/README.md) is active, this setting sets the interval in which the value the hunger value is decreased by 1. |  |
|       Party-Support: | boolean | `true` | - | Enables the support for parties from these plugins: [MCMMO](https://www.spigotmc.org/resources/mcmmo.2445/), [Heroes](https://www.spigotmc.org/resources/heroes.305/), [Ancient](http://dev.bukkit.org/bukkit-plugins/ancient-rpg/)  If you have any party plugins MyPet should support please [contact me](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/pages/contact/README.md). |  |
| **Hooks Settings** |  |  |  |  |  |
|   **Hooks:** |  |  |  |  |  |
|     AncientRPG: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the AncientRPG plugin like **parties**. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     Towny: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the Towny plugin. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     MCMMO: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the MCMMO plugin like **parties**. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     Factions: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the Factions plugin. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     Heroes: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the Heroes plugin like **minimum PvP level** or **parties**. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     WorldGuard: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the WorldGuard like **region flags**. If the owner of a pet can attack a player, the MyPet can attack this player too. This will not affect **Fly Zones** ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) because they can be disabled by just removing all zones. |  |
|     Residence: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the Residence. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     GriefPrevention: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the GriefPrevention. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     PvPManager: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player following the rules of the PvPManager. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|     Citizens: | boolean | `true` | - | Checks whether the target NPC can be damaged by a MyPet. NPCs can't be leashed. |  |
|     **MobArena:** |  |  |  |  |  |
|       Enabled: | boolean | `true` | - | Enables the MobArena hook. \(This allows the pet to deal damage in arenas\) |  |
|       RespectPvPRule: | boolean | `true` | - | If `true` pets will respects the PvP rules of an arena. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|       AllowPets: | boolean | `true` | - | If `true` pets will be able to enter arenas. |  |
|     **Minigames:** |  |  |  |  |  |
|       DisablePetsInGames: | boolean | `true` | - | Disables MyPets in any minigame completely. |  |
|     **PvPArena:** |  |  |  |  |  |
|       PvP: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player inside an arena. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|       DisablePetsInArena: | boolean | `true` | - | Disables MyPets in the arenas completely. |  |
|     **SurvivalGames:** |  |  |  |  |  |
|       PvP: | boolean | `true` | - | Checks whether the MyPet-owner can attack a target player inside a survival game match. If the owner of a pet can attack a player, the MyPet can attack this player too. |  |
|       DisablePetsInGames: | boolean | `true` | - | Disables MyPets in survival game matches completely. |  |
|     **MyHungerGames:** |  |  |  |  |  |
|       DisablePetsInGames: | boolean | `true` | - | Disables MyPets in survival game matches completely. |  |
|     **BattleArena:** |  |  |  |  |  |
|       DisablePetsInArena: | boolean | `true` | - | Disables MyPets in the arenas completely. |  |
|     **Vault:** |  |  |  |  |  |
|       Economy: | boolean | `true` | - | Enables the economy support. Used by ![$](https://github.com/xXKeyleXx/MyPet-Wiki/tree/07680434e1278c970819d5e9518888598106688b/wiki/images/premium.gif) trading, respawning and storing pets. |  |
|     **SkillAPI:** |  |  |  |  |  |
|       GrantExp: | boolean | `true` | - | This setting allows pets to gain XP when the owner gains SkillAPI XP. |  |
|       Disable-Vanilla-Exp: | boolean | `false` | - | This setting disables XP gain of normal sources at all. The only way pets can get any XP is when the owner gets XP via SkillAPI. |  |

