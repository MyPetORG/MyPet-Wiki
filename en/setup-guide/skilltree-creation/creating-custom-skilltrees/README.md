...
Skilltrees were designed to be customized to fit the needs of the server running MyPet. Because of this, the [MyPet Configurator](../../configurator/configurator/) includes a visual skilltree editor, which reads and writes the `.st.json` files used by the plugin.

To get started, run `/mypet editor` in-game and open the link it gives you, then select `Skilltrees` in the sidebar. See [Opening a Session](../../configurator/configurator/opening-a-session.md) for the full walkthrough, or [try the demo](https://editor.mypet-plugin.com/#demo) to explore with example data and no server.
...
Your changes are held in the browser until you apply them with `Review & Save`. See [Saving Changes](../../configurator/configurator/saving-changes.md).
