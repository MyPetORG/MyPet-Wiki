...
* CalculationMode
  * Type: string
  * Default: "Default"
  * Description: Which curve decides how much experience a pet needs per level. One of `Default` (also accepted as `MyPet`), `Linear`, `Power` or `Exponential`. Not case-sensitive. Unknown values fall back to `Default`. [More here](../systems/experience.md).
* #### Curve
