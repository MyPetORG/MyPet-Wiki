...
5. If everything succeeds, the server starts normally. **If anything fails, MyPet disables itself for that startup** rather than run on half-migrated data — see [If the upgrade fails](updating-from-3-to-4.md#if-the-upgrade-fails).
...
* **If you rely on the MyPet API** (a hook plugin, a custom integration), note that 4.0's API is a separate breaking change from this data migration — see [Migrating from v3.x](https://github.com/MyPetORG/MyPet-Wiki/tree/4.0/developer-guide/migrating-from-v3.md) if you or a plugin author you depend on maintains code against `mypet-api`.
