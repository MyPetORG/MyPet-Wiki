---
description: Frequently asked questions about MyPet 4, collected from the MyPet Discord.
...
Either way, name it exactly `Capybara` (or `Chameleon`) — the bundled skilltrees match on that name. Full walkthrough, including the Chameleon: [Bundled Models — Capybara & Chameleon](systems/custom-pet-models/bundled-pet-models.md).

**Why does `/petadmin create <player> mypet:capybara` say the pet type is invalid?**\
Because no custom creature named `Capybara` is defined yet, or its section is missing the `Host:` key. A `MyPet.Pets.<Name>` section only becomes a new pet type when it has `Host:` — without it the section is read as settings for an existing type. See [Bundled Models — Capybara & Chameleon](systems/custom-pet-models/bundled-pet-models.md).

**My model's animations have custom names — I could remap sit and the others, but where do I set the walk animation?**\
You don't set it in MyPet. MyPet only plays the **event** animations — `spawn`, `despawn`, `sit`, `sit_loop`, `unsit`, `attack` — and those names can be remapped per pet type (see [pet-config.yml → Model animations](configuration/pet-config.yml/#model-animations)). Movement animations like walking and idling are handled by the rendering plugin itself (ModelEngine / BetterModel / ItemsAdder), not by MyPet — so either rename your walk animation to the name that plugin expects (usually `walk`), or check whether the plugin lets you target a different animation name. See [Custom Pet Models](systems/custom-pet-models/#animations).

## Updating

**Can I update my server from MyPet 3.x (e.g. 3.14.2) straight to 4.0, or will there be incompatibilities and data loss?**\
Yes, you can update directly. MyPet 4 was designed for automatic conversion — replace the plugin jar, start the server, and all of your plugin data (pets, levels, names, inventories) is migrated on its own. Take a backup first anyway: the migration is one-way, with no downgrade path back to 3.x. Full details in [Updating from MyPet 3 to 4](updating-from-3-to-4.md).
