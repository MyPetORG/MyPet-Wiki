...
Download [Rhino](https://github.com/mozilla/rhino/releases), rename the jar to `rhino.jar` and put the file into the MyPet folder.
{% endstep %}

{% step %}
### Set CalculationMode

Set `LevelSystem.CalculationMode` to `JavaScript` in the `config.yml`.
{% endstep %}

{% step %}
### Edit the script

Edit the `exp.js` to your likings.
{% endstep %}
{% endstepper %}
...
{% tabs %}
{% tab title="Pet Type" %}
{% code title="exp.js — pet type example" %}
```javascript
function getExpByLevel(level, petType, worldGroup) {
  var exp = 10;
  if(petType == "Cow") {
...
{% code title="exp.js — world group example" %}
```javascript
function getExpByLevel(level, petType, worldGroup) {
  var exp = 10;
