...
Across all six methods, MyPet plays a handful of **animations** on the model automatically — on spawn, despawn, sit/unsit, and attack. If your models name those animations the MyPet defaults (`spawn`, `despawn`, `sit`, `sit_loop`, `unsit`, `attack`) they work out of the box; otherwise you can remap the names per pet type. Walking, idling, and other movement animations are handled by the rendering plugin itself, not by MyPet. See [pet-config.yml → Model animations](../configuration/pet-config.yml/README.md#model-animations) for the full list and how to override names.
