...
{% hint style="success" %}
**Name the section exactly `Capybara`** (and `Chameleon` for the other). MyPet's bundled skilltrees list those names in their eligible mob types — a Capybara named `Capy` or `MyCapybara` gets **no** default skilltrees. See [Which skilltrees they get](#which-skilltrees-they-get).
{% endhint %}
...
5. Name the creature `Capybara`, pick a **host mob** (see [`Host:`](#choosing-a-host) — `Pig` is a good default), set HP/Speed, then **Create**.
...
Both bundled models already use MyPet's default animation names — `spawn`, `despawn`, `sit`, `sit_loop`, `unsit`, `attack` — plus `walk`, `run`, `idle`, and `jump` for the rendering plugin. Nothing to remap. See [Animations](custom-pet-models.md#animations).
...
* [Custom Pet Models](custom-pet-models.md) — the full feature, all six methods
